...
from models.sensor_fusion_origin import SensorFusionSelfSupervised
from utils import (
    kl_normal,
    realEPE,
    compute_accuracy,
...
class selfsupervised_origin:
    ...
    def validate(self, i_epoch):
        ...
        for i_iter, sample_batched in enumerate(self.dataloaders["val"]):
            ...
            flow_loss, contact_loss, is_paired_loss, contact_accuracy, is_paired_accuracy, kl = (
                results_val
            )
    ...
    def loss_calc(self, sampled_batched):
        ...
        if self.deterministic:
            paired_out, contact_out, flow2, mm_feat = self.model(
                image, force, proprio, action
            )
            kl = torch.tensor([0]).to(self.device).type(torch.cuda.FloatTensor)
        else:
            paired_out, contact_out, flow2, mm_feat, mu_z, var_z, mu_prior, var_prior = self.model(
                image, force, proprio, action
            )
        ...
        unpaired_total_losses = self.model(
            unpaired_image, unpaired_force, unpaired_proprio, action
        )
