...
from models.base_models_origin.encoders import (
    ProprioEncoder,
    ForceEncoder,
    ImageEncoder,
)
from models.base_models_origin.decoders import (
    OpticalFlowDecoder,
)
...
class SensorFusion(nn.Module):
    ...
    def forward_encoder(self, vis_in, frc_in, proprio_in, action_in):
        ...
        if self.deterministic:
            ...
            mm_f1 = torch.cat([img_out, frc_out, proprio_out], 1).squeeze()
            ...
            m_vect = torch.cat(
                [mu_z_img, mu_z_frc, mu_z_proprio, mu_prior_resized], dim=2
            )
            var_vect = torch.cat(
                [var_z_img, var_z_frc, var_z_proprio, var_prior_resized],
                dim=2,
            )
        ...
        if self.encoder_bool or action_in is None:
            if self.deterministic:
                return img_out, frc_out, proprio_out, z
            else:
                return img_out_convs, img_out, frc_out, proprio_out, z
        else:
            # action embedding
...
class SensorFusionSelfSupervised(SensorFusion):
    ...
    def forward(
        ...
        if self.encoder_bool:
            ...
            z = self.forward_encoder(vis_in, frc_in, proprio_in, action_in)
            return z

        elif action_in is None:
            z = self.forward_encoder(vis_in, frc_in, proprio_in, None)
            ...
            if self.deterministic:
                img_out_convs, mm_act_feat, z = self.forward_encoder(
                    vis_in, frc_in, proprio_in, action_in
                )
            else:
        ...
        # -------------------------#
        optical_flow2 = self.optical_flow_decoder(
