...
from models.base_models_depth.encoders import (
    ProprioEncoder,
    ForceEncoder,
    ImageEncoder,
    DepthEncoder,
)
from models.base_models_depth.decoders import (
    OpticalFlowDecoder,
    EeDeltaDecoder
)
...
class SensorFusion(nn.Module):
    ...
    def __init__(
        ...
        if deterministic:
            ...
            # print("***********deterministic************INITIALIZED")
            if weighted:
                ...
                self.ft_weight = torch.nn.Parameter(torch.FloatTensor(1), requires_grad = True)
                self.prop_weight = torch.nn.Parameter(torch.FloatTensor(1), requires_grad = True)
                self.rgb_weight.data.fill_(1.0)
                self.depth_weight.data.fill_(0.8)
                self.ft_weight.data.fill_(0.6)
                self.prop_weight.data.fill_(0.5)
    ...
    def forward_encoder(self, vis_in, frc_in, proprio_in, depth_in, action_in):
        ...
        if self.deterministic:
            if self.weighted: 
                total_weight = self.rgb_weight + self.depth_weight + self.ft_weight + self.prop_weight
                img_out = self.rgb_weight/total_weight * img_out
                depth_out = self.depth_weight/total_weight * depth_out
                frc_out = self.ft_weight/total_weight * frc_out
...
class SensorFusionSelfSupervised(SensorFusion):
    ...
    def __init__(
        ...
        self.deterministic = deterministic
        self.weighted = weighted
        # print("*********************supervised : self.weighted***************: ", self.weighted)
        # -----------------------
